...
```mermaid
flowchart TB
    subgraph Frontend ["🖥️ Frontend (web/)"]
        direction LR
        HTML["index.html"]
        CSS["styles.css"]
        JS["app.js"]
    ...
    subgraph Tools ["🔧 Tools & Guardrails (tools.py)"]
        ...
        G1["🛡️ Refund > ₹25K → BLOCKED"]
        G2["🛡️ Duplicate Refund → BLOCKED"]
    end

    subgraph Backends ["🗄️ Mock Backends"]
        DB["mock_txn_db.py\n(Transaction DB)"]
...
| **⚙️ Live Agent Activity** | Every tool call with args, results, and latency in real-time |
